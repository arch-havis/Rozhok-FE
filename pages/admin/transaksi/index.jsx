...
const Index = () => {
    ...
    const getDataTransaksi = async () => {
        try {
            const response = await axios.get("https://altagp3.online/transaksi/admin", {
                headers: {
                        ...
                        {dataTransaksi.map((data, i) => {
                            ...
                            return (
                                <tr key={data.id_transaksi}>
                                    ...
                                    <td>{data.status}</td>
                                    <td>
                                        <AiTwotoneEdit className="fs-4 text-lime user-select-auto" onClick={() => DetailTransaksi(data.id_transaksi)} />
                                    </td>
                                </tr>
                            );
                        })}
